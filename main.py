...
class Window:
    def __init__(self, width: int, height: int, title: str):
        ...
        self._window = glfw.create_window(
            width, height, 'AirBUS A380 Modeling', None, None)
        ...
        # Set window as current context_window
        ...
        # Making camera look slightly up to the sky form the centre of the plane
        self._static_target: v3 = v3([0, 2.0, 0])
        # Default camera position, Camera is always looking along Z-axis ; called eye space
        self._default_eye: v3 = v3([0, 8, 10])
        ...
        self.scene = {
            ...
            "Runway": LoadedObject("data/platform2.obj", 0.5, -2.1, 0, 1.5),
            "Center_Plane": LoadedObject("data/rocket2.obj", 0, 2.1, 0, scale=0.4),
            # "Moving_Plane": LoadedObject("data/rocket2.obj", scale=0.25),
        }

        # Lighting
        # sphere to represent point light sources
        self._point_light_obj = LoadedObject("data/uv_sphere.obj")
        ...
        # Relative offset from moving plane
        self.spot_light_offset = v3([-1.0, 0.0, 0.0])
        # Default direction (same as moving plane)
        self.spot_light_def_dir = v3([0.0, 0.0, 0.0])
        ...
        self.spot_light = SpotLight(amb=v3([0.0, 0.0, 0.0]), dif=v3([0.0, 1.0, 0.5]), spe=v3([0.0, 1.0, 0.5]),
                                    # position is marked at headlight
    ...
    def _update_view(self) -> None:
        ...
        self.view_matrix = lookAt.create_look_at(
            self._eye, self._target, self._up)
    ...
    def _update_projection(self) -> None:
        ...
        self.projection_matrix = Perspective_projection.create_perspective_projection(
            self._fov, a, self._near, self._far)
        self.current_shader.set_projection(
            self.projection_matrix)  # Uploading to Shader
    ...
    def _on_key_input(self, _window, key, _scancode, action, _mode) -> None:
        ...
        if action != glfw.PRESS:
            return
        cam = {glfw.KEY_1: "static",
               glfw.KEY_2: "following", glfw.KEY_3: "moving"}
        if key in cam:
    ...
    def _set_daytime(self):
        ...
        c = self._background_color_day * \
            (1 - blend_factor) + self._background_color_night * blend_factor

        # Changing Intensity of Day Light
        self.sun_moon._diffuse_set_daytime = 0.9 * c
    ...
    def _move_objects(self) -> None:
        ...
        translation = Arithmetic.create_from_translation(
            v3([0, math.sin(time) + 0.23, 0]))  # move up_down (in y dir)

        model = Arithmetic.multiply(
            translation, self.scene["Center_Plane"].pos)  # up-down movement
        self.scene["Center_Plane"].model = Arithmetic.multiply(
            rot_y, model)  # rotation
        ...
        # o.model = Arithmetic.multiply(
        #     Arithmetic.create_from_y_rotation(-self.spot_light_angle_offset_x - math.pi), o.model)
        # o.model = Arithmetic.multiply(
        #     Arithmetic.create_from_z_rotation(-self.spot_light_angle_offset_y), o.model)

        # Move and orientate spotlight relatively to Moving_Plane
        # pos = Arithmetic.multiply(Arithmetic.create_from_translation(
        #     self.spot_light_offset), o.model)
    ...
    def _process_camera(self) -> None:
        ...
        if self.sel_camera == "static":
            ...
            # Static camera needs to be calculated only once.
            self.update_camera = False
        elif self.sel_camera == "following":
            self._eye = self._default_eye
            self._target = Vector3.from_matrix44_translation(
                self.scene["Runway"].model)
        elif self.sel_camera == "moving":
            m = Arithmetic.multiply(Arithmetic.create_from_translation(
                v3([-0.9, 0.0, 0])), self.scene["Runway"].model)  # plane cockpit view
